...
#### 1. Создадим бэкэнд в S3 (необязательно, но крайне желательно).
   ...
   * Создал бакет в Yandex.Cloud

---
### 2. Инициализируем проект и создаем воркспейсы. 

   * Выполнил `terraform init`: создан локальный файл со стейтами.  
   * Создал два воркспейса `stage` и `prod`:
   ```shell
...
использовались разные `instance_type`.
   ...
   * Чтобы при изменении типа инстанса не возникло ситуации, когда не будет ни одного инстанса добавил параметр
жизненного цикла `create_before_destroy = true`.

   * Вывод команды `terraform plan` для воркспейса `prod`:
     
```shell
kaa@kaa-vm01:~/devops/tf$ terraform plan
...
Terraform will perform the following actions:
  ...
  # yandex_compute_instance.vm2["prod_vm_1"] will be created
  ...
  # yandex_compute_instance.vm2["prod_vm_2"] will be created
  + resource "yandex_compute_instance" "vm2" {
      + created_at                = (known after apply)
      + folder_id                 = (known after apply)
      + fqdn                      = (known after apply)
